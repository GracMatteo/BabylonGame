...
class Player {
  ...
  async init() {
    ...
    if (DEBUG_MODE) {
      ...
      // Créer un matériau rouge pour l'ellipsoïde
      ...
      // Créer un maillage d'ellipsoïde réel à la place des lignes
      ...
      this.ellipsoidMesh.position.y = offsetY;
      this.ellipsoidMesh.isPickable = true;
      
      // Ajouter des attributs pour le rendre visible dans l'inspecteur
      this.ellipsoidMesh.showBoundingBox = true;
      this.ellipsoidMesh.wireframe = true;
      
      // Créer également l'ellipsoïde en lignes comme backup visuel
      const points = [];
